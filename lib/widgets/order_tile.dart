      ...
      child: Card(
        child: ExpansionTile(
          key: Key(order.documentID),
            initiallyExpanded: order.data["status"] != 4? true : false,
            title: Text(
              "#${order.documentID.substring(order.data.length - 7, order.data.length)} - "
                  "${states[order.data["status"]]}",
              style: TextStyle(color: order.data["status"] != 4? Colors.grey[850] : Colors.greenAccent),
            ),
            children: <Widget>[
              Padding(
                padding: EdgeInsets.only(left: 16.0, right: 16.0, top: 0,bottom: 8),
                child: Column(
